...
import 'package:weather_app/feature/weather/data/remote/models/weather_more_information_model.dart';
...
abstract class BaseWeatherRemoteDataSource {
  ...
  Future<WeatherMoreInformationModel> getWeatherMoreInformation({
    required double lat,
    required double lon,
  });
}
...
class WeatherRemoteDataSource extends BaseWeatherRemoteDataSource {
  ...
  Future<WeatherMoreInformationModel> getWeatherMoreInformation(
    ...
    final response = await Dio().get(
        "${EndPoints.baseUrl}/forecast?id=524901&lat=$lat&lon=$lon&appid=${EndPoints.apiKey}");
    if (response.statusCode == 200) {
      return WeatherMoreInformationModel.fromJson(response.data);
    } else {
      throw ServerException(
          errorMessageModel: ErrorMessageModel.fromJson(response.data));
    }
